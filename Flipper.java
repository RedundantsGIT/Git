...
public class Flipper extends PollingScript<org.powerbot.script.rt6.ClientContext> implements PaintListener, MessageListener {
    ...
    public void poll() {
        ...
        switch(state()){
            ...
            case MENU:
                ...
                break;
            case CONTINUE:
                if (ctx.widgets.component(1191, 10).text().contains("Can you sell me some furs?") || ctx.widgets.component(1191, 10).text().contains("Yeah, okay, here you go.")) {
                ...
                }else if (ctx.widgets.component(1184, 10).text().contains("Yeah, sure. They're 20 gold coins each.")){
                    STATUS = "Select Continue2";
                    ctx.input.send(" ");
                    Condition.wait(new Callable<Boolean>() {
                        @Override
                        public Boolean call() throws Exception {
                            return ctx.widgets.component(1188, 6).text().contains("Yeah, okay, here you go.");
                        }
                    }, 250, 20);
                }else{
    ...
    private State state() {
        ...
        if(ctx.widgets.component(1191, 11).valid() || ctx.widgets.component(1184, 11).valid() || WIDGET_MENU2.valid()){
