...
@Manifest(name = "rPlanker", description = "Planks logs in Varrock", hidden = true)
public class rPlanker extends PollingScript implements PaintListener,
	...
	private static final int NORMAL_PLANK_ID = 960, OAK_PLANK_ID = 8778,
				...
				if (!PlankMenu.getComponent(WIDGET_CHOSEN).isOnScreen()) {
					...
					if (didInteract()) {
						Condition.wait(new Callable<Boolean>() {
							@Override
							public Boolean call() throws Exception {
								return PlankMenu.getComponent(WIDGET_CHOSEN)
										.isOnScreen();
							}
						}, 250, 20);
					}
	...
	public void repaint(Graphics g) {
		...
		g.drawString("Runtime: " + hours + ":" + minutes + ":" + seconds, 351,
				100);
	...
	public class rPlankerGUI extends JFrame {
		...
		private void initComponents() {
			...
			PlanksToMake.setModel(new DefaultComboBoxModel<String>(
					new String[] { "Normal", "Oak", "Teak", "Mahogany" }));
